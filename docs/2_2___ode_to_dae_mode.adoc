...
The variable `enableDAEModeParameter` is declared in the `modelDescription.xml` as follows:
...
and must be referenced by value reference within the FMI-LS-DAE manifest file via the <<EnableDAEParameter>> element, see <<layered-standard-manifest-file>>.

The `name` and `valueReference` is assigned freely by the FMU exporter; the importer locates the parameter by its `valueReference` declared in the <<EnableDAEParameter>> element of `fmi-ls-manifest.xml`.
...
The structural parameter `enableDAEModeParameter` must not be referenced in `<Dimension>` elements.
