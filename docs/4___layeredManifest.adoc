...
Each <<AlgebraicVariable>> has the following attributes.
...
|`nominal`
|Optional attribute containing the value reference for the variable present in the `ModelVariables` of the `modelDescription.xml` that represents the nominal value of the algebraic variable.
|====

NOTE: In the core Model Exchange standard there exists the API function *https://fmi-standard.org/docs/3.0.2/#fmi3GetNominalsOfContinuousStates[fmi3GetNominalsOfContinuousStates]*, which only holds for the continous time states. This `nominal` attribute of the `<AlgebraicVariable>` element should be seen as an extension targeting the algebraic variables, if it is the case that they would change during integration.

If no `nominal` attribute is present under the `<<AlgebraicVariable>>` element, and no `nominal` attribute is present for the algebraic variable under `ModelVariables` of the `modelDescription.xml`, then the algebraic variable has a default nominal of 1.0.
